...
func Test_Deployments_Delete(t *testing.T) {
	...
	mux.HandleFunc("/api/v1/deployments/myenv/myns/mydep", func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, http.MethodDelete, r.Method)
		fmt.Fprint(w, `{"message": "deleted"}`)
	})

	result, err := client.Deployments.Delete("mydep", "myns", "myenv")
...
func Test_Deployments_SaveStatus(t *testing.T) {
	...
	mux.HandleFunc("/api/v1/deployments/myenv/myns/myapp/status", func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, http.MethodPut, r.Method)
